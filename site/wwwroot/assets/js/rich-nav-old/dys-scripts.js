...
$.ajax({
    url: "/assets/json/product_nav/product-nav.json",
    type: "GET",
    dataType: "json",
    cache: false,
    success: function(data) {
        ...
        $(document).ready(function() {
        	...
        	//add spans to main navigation for styling
        	$(".main-menu-wrapper ul li").each(function(){
        	   var menu_text = $(this).html();
        	   $(this).html("<span>" + menu_text + "</span>");
        	});
        	
        	$(document).on("click", ".main-menu-wrapper li", function(e){
        ...
        //             
                    ...
                    // $(".filter-by-button").removeClass("on");
                    // $(".clear-filters-button").addClass("disabled");
                    // console.log("here");
                    // filter_and_sort_dd17_menu();
                    
                    $(".main-menu-wrapper li").removeClass("active");
                    $(".main-nav-dropdown:visible").slideToggle(400, function(){
                        $(".nav-lasers").html();
            	    ...
            	    if(!$(this).hasClass("active")){
                	    ...
                	    $(".main-nav-dropdown:visible").slideToggle();
                	    switch(li_class){
                	        case 'laser-nav-display':
                	           size_dd17_menus(0);
                	           $(".nav-lasers").slideToggle();
                	           break;
                	        case 'app-nav-display':
                	           $(".nav-applications").slideToggle();
                	           break;
                	        case 'support-nav-display':
                	           $(".nav-support").slideToggle();
                	           break;
                	        case 'company-nav-display':
            ...
            $(document).on("click", ".menu-dd17 .panels-container .panel .thead .tr .th", function(e) {
                var category = $(this).attr("data-category");
                var category_index = $(this).attr("data-category-index");
                var sub_category_index = $(this).attr("data-sub-category-index");
                var sort_dir = $(this).attr("data-sort");
                var sorted_by = $(this).attr("data-sorted-by");
                var colum_name_formatted = format_name($("span", this).html(), "_");
                ...
                if(colum_name_formatted != "compare"){
                    ...
                    if(sorted_by == 1){ // the table is already sorted by this column
                        ...
                        $(this).siblings().attr("data-sorted-by", "0");
                        $(this).siblings().attr("data-sort", "asc");
                        $(this).attr("data-sorted-by", "1");
                        var new_sort_dir = "asc";
                    }
            ...
            $(document).on("click", ".menu-dd17 .tbody .tr .td input, .menu-dd17 .tbody .tr .td label", function(e){
               ...
               if($("input[type='checkbox']:checked", this_tbody).length){
                   this_tbody.siblings(".compare-buttons").children(".compare-selected-button").removeClass("disabled");
                   this_tbody.siblings(".compare-buttons").children(".compare-reset-button").removeClass("disabled");
               }else{
                   this_tbody.siblings(".compare-buttons").children(".compare-selected-button").addClass("disabled");
                   this_tbody.siblings(".compare-buttons").children(".compare-reset-button").addClass("disabled");
               }
            });
            ...
            $(document).on("click", ".menu-dd17 .filter-by-button", function(e){
                e.stopPropagation();
        ...
        function build_dd17_products_menu(){
            ...
            for(var i = 0; i < categories.length; i++) {
                ...
                if(categories[i].hasOwnProperty('sub_categories')){
                    ...
                    for(var s = 0; s < categories[i].sub_categories.length; s++) {
                        ...
                        panel_output += '<li><a ' + active_class + ' href="javascript:void(0);" data-category="' + categories[i].name + '" data-category-index="' + i + '" data-sub-category-index="' + s + '"><img src="' + categories[i].sub_categories[s].image + '" alt="' + categories[i].sub_categories[s].name + '"><span>' + categories[i].sub_categories[s].name + '</span></a></li>';
                    }
                ...
                if(!categories[i].hasOwnProperty('sub_categories')){
                    for(var c = 0; c < categories[i].columns.length; c++) {
                        ...
                        if(categories[i].columns[c] != "Compare"){
                            if(c == 0){
                                ...
                                var filter_html = '<div class="filter-by">' +
                                                        '<div class="filter-by-button"><i class="fa fa-ellipsis-h" aria-hidden="true"></i></div>' +
                        ...
                        panel_output +=         '<div class="th col_' + categories[i].columns.length + '" data-category="' + category_formatted + '" data-category-index="' + i + '" data-sub-category-index="none" ' + sort_attributes + '><span>' + categories[i].columns[c] + '</span>' + filter_html + '</div>';
                ...
                // if there are sub categories get the columns for the first sub category
                    ...
                    for(var c = 0; c < categories[i].sub_categories[0].columns.length; c++) {
                        ...
                        panel_output +=         '<div class="th col_' + categories[i].sub_categories[0].columns.length + '" data-category="' + category_formatted + '" data-category-index="' + i + '" data-sub-category-index="0" data-sort="asc" data-sorted-by="' + sorted_by + '"><span>' + categories[i].sub_categories[0].columns[c] + '</span>' + filter_html + '</div>';
            ...
            for (var k in filters_array){
                ...
                for (i = 0; i < this_filter_array.length; i++) {
                    this_filter_html += '<div class="filter-option"><i class="fa fa-check-square-o" aria-hidden="true"></i><span>' + this_filter_array[i] + '</span></div>'
                }
        ...
        function filter_and_sort_dd17_menu(){
            ...
            var visible_thead = $(".menu-dd17 .thead:visible");
                ...
                for (var f in filter_array){
                    ...
                    if(filter_array[f].indexOf("||") !== -1){
                        ...
                        for(var v = 0; v < filter_values_array.length; v++) {
                            if(items[r]['columns'][f] == $('<textarea />').html(filter_values_array[v]).text()){
                                this_col_ok = 1;
                            }
                        }
                    }else{
                        if(items[r]['columns'][f] == $('<textarea />').html(filter_array[f]).text()){
                            this_col_ok = 1;
                        }
                    }
                            ...
                            '<div class="mobile-menu-dd17">' +
                                ...
                                '<div class="mobile-header group">' +
                                    ...
                                    '<div class="mobile-header-btn vcenter">' +
                                        '<i id="js-mobile-icon" class="fa fa-bars" aria-hidden="true"></i>' +
                                    '</div>' +
                                '</div>' +
