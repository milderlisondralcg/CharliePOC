    ...
    while($row = $result->fetch_assoc()){
        ...
        if(strpos($row["categories"],$type)!== FALSE || $type == "All"){
            foreach($GLOBALS["catTemp"] as $value){
                if($value != "" && $value != "Sales" && $value != "Service" && $value != "Training"){
...
function compileContent($content){
    ...
    for($int = 0; $int < count($content["title"]); $int++){
        $title = ""; $address = ""; $phone = ""; $email = ""; $website = ""; $additional = ""; $countries = ""; $priority = "";
        foreach($content as $key => $value){
            ...
            if($key == "phone" && $value[$int] != ""){
                $phone = "<span class='vendor_phone'>".$value[$int]."</span>";
            }
            ...
            if($key == "countries" && $value[$int] != ""){
                $countries = "<span class='vendor_supported'>Locations Supported: ". str_replace(",", ", ", $value[$int])."</span></div>";
            }
            if($key == "priority" && $value[$int] != ""){
                $priority = $value[$int];
            }
        }
        $outArray[$title . $address . $phone . $email . $website . $additional . $countries] = $priority;
    }
    asort($outArray);
    $i = 0;
    $len = count($outArray);
